// Shared `window.__ableInspect` bridge, injected into pages the Explore
// workbench needs to inspect. The same bridge is also hand-authored inline in
// `public/explore-demo.html` (a static asset that can't import a TS module at
// request time) — keep the two in sync if either changes. `tests/bridge-script.test.ts`
// smoke-tests this copy directly in jsdom.
//
// Exposes both a direct `window.__ableInspect` object (used when the iframe is
// same-origin — e.g. proxied through `/api/preview-proxy`) and a `postMessage`
// protocol (`able:*` message types) for any future cross-origin embedding.
