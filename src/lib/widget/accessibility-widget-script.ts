/**
 * ScanA11y embeddable accessibility widget — a standalone, dependency-free
 * script any website can add via a single <script> tag so ITS OWN visitors
 * get a UX4G-style accessibility options panel (profiles, contrast, text
 * size, reading aids) that applies live to the real page.
 *
 * Distinct from AccessibilityOptionsPanel (src/components/workbench/explore
 * /accessibility-options.tsx) and ABLE_INSPECT_BRIDGE_SCRIPT
 * (src/lib/explore/bridge-script.ts), which are our OWN internal testing
 * tools scoped to our proxied preview iframe only (see AUTOPILOT.md: "never
 * injected onto the real target site"). This file is the opposite of that
 * constraint by design — it exists specifically to BE injected onto real
 * third-party sites, at the site owner's own choice.
 *
 * The DOM-manipulation core (applyProfile) mirrors
 * applyAccessibilityProfile in bridge-script.ts — same behavior, adapted to
 * run directly against the host document instead of through postMessage
 * (this script IS the page, not an iframe being inspected from outside).
 *
 * Served as a plain JS file at /widget.js (src/app/widget.js/route.ts).
 * Embed snippet:
 *   <script src="https://scana11y-nine.vercel.app/widget.js" async
 *     data-position="bottom-right" data-accent="#2563eb"></script>
 */

export const ACCESSIBILITY_WIDGET_SCRIPT = `
(function () {
  "use strict";
